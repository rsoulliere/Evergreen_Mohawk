...
<xsl:stylesheet version="1.0"
 ...
 >
	...
	<xsl:variable name="t-lang">en-CA</xsl:variable> <!-- the ISO 639 code the the language that the text (the texts below) are in -->
	...
	<xsl:if test="atom:icon">
				...
				<link rel="stylesheet" type="text/css" title="default" media="screen">
					<xsl:attribute name="href"><xsl:value-of select="concat($base_dir,'mobile.css')"/></xsl:attribute>
				</link>
				<!-- rel links -->
...
<h1><xsl:value-of select="$title" /></h1>
				...
				<div id="searchdiv">
					<form method="GET">
						<xsl:attribute name="action"><xsl:value-of select="concat($base_dir, 'opensearch/1.1/', $lib, '/htmlmobile-full')" /></xsl:attribute>
						<b>Search:</b>
						<input class="searchbox" type="text" name="searchTerms" value="{$searchTerms}"/>
						<select class="formElem" ame="searchClass">
						...
						<input class="formElem" type="submit" value="Go!"/>
					</form>
					<br/>
				</div>
	...
	<xsl:template match="atom:entry | item | //rss1:item | //rss9:item"> <!-- match="" must match the select="" earlier on -->
		<xsl:variable name="url"><xsl:call-template name="resolvelink"><xsl:with-param name="url" select="(atom:link[not(@rel) or @rel='mobile' or @rel='']/@href | link | guid[@isPermaLink='true'] | rss1:link | rss9:link)[1]" /></xsl:call-template></xsl:variable>
		<!-- item title -->
		<dt>
			<xsl:choose>
			...
			<dd class="desc">
				<xsl:if test="string-length($url)&gt;0">
					<a href="{$url}" style="text-decoration: none;">
						<img align="left" style="margin:5px; border: 0px;" height="0" width="0">
		...
		<xsl:if test="string-length($name)&gt;0">
			...
			<xsl:variable name="url">
				<xsl:choose>
					...
					<xsl:otherwise>
						<xsl:value-of select="concat($base_dir, 'opensearch/1.1/', $lib, '/htmlmobile-full/subject?searchTerms=', $name)" />
					</xsl:otherwise>
				</xsl:choose>
			</xsl:variable>
			<xsl:choose>
		...
		<xsl:if test="string-length($showname)&gt;0">
			<xsl:choose>
				...
				<xsl:otherwise>
					<a>
						<xsl:attribute name="href">
							<xsl:value-of select="concat($base_dir, 'opensearch/1.1/', $lib, '/htmlmobile-full/author?searchTerms=', $showname)" />
						</xsl:attribute>
