  ...
  <xsl:template match="/">
     <html>
       <head>
         ...
         <meta content="minimum-scale=1.0, initial-scale=1.0, width=device-width, maximum-scale=0.66, user-scalable=yes" name="viewport" />
	<link href="{$base_dir}/htmlcardmobile.css" rel="stylesheet" type="text/css" />
...
<!--     <span class="bottom">-->
 <!--     <xsl:apply-templates select="marc:controlfield[@tag='001']"/>
    ...
    <br/> 
  <p/>   <xsl:apply-templates select="xhtml:link[@rel='otherFormat' and contains(@href,'format=')]"/>
 </xsl:template>
...
<xsl:value-of select="@barcode"/>
	...
	<br/>Campus: <xsl:value-of select="hold:circlib"/> |	
	Location: <xsl:value-of select="hold:location"/> |
	Status: <xsl:value-of select="hold:status"/>
...
<!--
    <xsl:if test="starts-with(@tag, '1')">
      <p class="mainheading">
       <a>
        <xsl:attribute name="href">
         <xsl:value-of select="concat('/opac/extras/opensearch/1.1/-/htmlmobile-full/author?searchTerms=',.)"/>
	</xsl:attribute>
        <xsl:value-of select="."/>
       </a>
      </p>
    ...
    <xsl:if test="@code!='2'">    
     ...
     <xsl:choose>
      <xsl:when test="./@code!='v' and ./parent::*[starts-with(@tag,'6')]">
       <a>
        <xsl:attribute name="href">
         <xsl:value-of select="concat('/opac/extras/opensearch/1.1/-/htmlmobile-full/subject?searchTerms=',.)"/>
	</xsl:attribute>
        <xsl:value-of select="."/>
       </a>
      </xsl:when>
