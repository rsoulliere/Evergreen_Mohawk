...
*/
	...
	if( is_mr )  {
		...
		pic.parentNode.setAttribute("href", buildOPACLink(args));
		$n(r, "result_counter").appendChild(text(pos +1 + getOffset() + '\n'));
		unHideMe($n(r,'place_hold_span'));
	...
	if(! is_mr ) {
		...
		if(!isNull(rec.physical_description()) ) {
		 ...
		 if(!isNull(rec.online_loc()) ) {
                        unHideMe( $n(r, "result_table_extra_span"));
                        var links = rec.online_loc();
			  	...
			  	$n(r, "result_table_online_span").appendChild(elem('br'));
                                $n(r, "result_table_online_span").appendChild(elem('a', {href:href,'target':'_blank','class':'copy_link'}, displayLabel));
                        }
                }
